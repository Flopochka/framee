...
let tonConnectUI;
try {
  // Безопасное формирование twaReturnUrl
  let twaReturnUrl = "https://t.me/framestars_bot?startapp";
  if (
    window.Telegram &&
    ...
    twaReturnUrl = `https://t.me/${window.Telegram.WebApp.initDataUnsafe.user.username}?startapp=${window.Telegram.WebApp.initDataUnsafe.start_param}`;
  } else {
    console.warn(
      "[wallet] Не удалось получить корректный twaReturnUrl из Telegram WebApp, используется дефолтный URL."
    );
  }
